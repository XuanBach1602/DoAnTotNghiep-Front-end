...
const OrderDetail = () => {
  ...
  const updateStatus = async (status, shipStatus) => {
    try {
      var response = await putAsync(`/api/Order/UpdateStatus?id=${order?.id}&status=${status}&shipStatus=${shipStatus}`);
  ...
  return (
    <div className="place-order-container">
      ...
      <div className="placeorder-main">
        ...
        {order?.status == 2 && order?.shipStatus == 0 && <h3>Not shipped yet</h3>}
          ...
          <Row className="form-data" gutter={[16, 1]}>
            ...
            <Col span={12}>
              <FormItem label="Phone Number" style={{ marginBottom: 0 }}>
                <Input value={order?.phoneNumber} placeholder="Phone Number" readOnly />
              </FormItem>
            </Col>
            <Col span={12}>
            ...
            <Col span={12}>
              <FormItem label="Created Date" style={{ marginBottom: 8 }}>
                <Input value={order?.createdDate} placeholder="Created Date" readOnly />
              </FormItem>
            </Col>
        ...
        <div style={{ display: "flex", gap: "20px", marginTop: "20px", marginBottom:"15px" }}>
      {(order?.status === 1 || (order?.status === 2 && order?.shipStatus === 0)) &&
      ...
      {order?.status === 2 && order?.shipStatus === 0 &&
        <div style={{ display: "flex", gap: "20px" }}>
          <Popconfirm
            title="Bạn có chắc chắn muốn gửi hàng?"
            okText="Yes"
            cancelText="No"
      ...
      {(order?.status === 2 && order?.shipStatus === 1) &&
        <div style={{ display: "flex", gap: "20px" }}>
          <Popconfirm
            title="Bạn có chắc chắn muốn hoàn thành đơn hàng?"
            okText="Yes"
            cancelText="No"
        ...
        <div className="total-container">
          <div style={{ fontSize: "24px", display:"flex", justifyContent:"flex-end" }}>
          <div style={{width:"180px",textAlign:"left"}}>Temporary Total: </div><div style={{ color: "orange", minWidth:"100px" }}>${order?.tempTotal}</div>
          </div>
        </div>
        <hr />
        <div className="total-container">
        <div style={{ fontSize: "24px", display:"flex", justifyContent:"flex-end" }}>
        <div style={{width:"180px",textAlign:"left"}}>Reduce:</div> <div style={{ color: "orange",display:"inline-block", minWidth:"100px" }}>${order?.reduce}</div>
          </div>
        </div>
        <hr />
        <div className="total-container">
        <div style={{ fontSize: "24px", display:"flex", justifyContent:"flex-end" }}>
           <div style={{width:"180px",textAlign:"left"}}> Total:</div> <div style={{ color: "orange",display:"inline-block", minWidth:"100px" }}>${order?.total}</div>
          </div>
        </div>
      </div>
    </div>
