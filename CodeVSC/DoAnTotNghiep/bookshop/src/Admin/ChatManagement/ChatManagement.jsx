...
const ChatManagement = () => {
      ...
      setConversationList((prevList) =>
        prevList.map((conv) =>
          conv.id === conversation.id
            ? { ...conv, lastMessage: tempMessage, lastMessageTime: new Date() }
            : conv
        )
      );
      setTempMessage("");
  ...
  const updateSeenStatus = async (conversation) => {
    try {
      await putAsync(
        "/api/Chat/UpdateSeenStatus?conversationId=" + conversation.id
      );
      setConversationList((prevList) =>
        prevList.map((conv) =>
  ...
  const createConversation = async (userId) => {
    try {
      var result = await postAsync(
        "/api/Chat/CreateConversation?userId=" + userId
      );
      setSearchTerm("");
      setConversationList((prevState) => [result, ...prevState]);
  ...
  return (
    <div className="chat-managemnet-controller">
      <div className="chat-left">
          ...
          {searchTerm && (
            <div className="user-list">
              {userList.map((user, index) => (
                ...
                >
                  <img
                    src={user.avatarUrl ? user.avatarUrl : "../user.png"}
                    alt={user.name}
                    className="avatar"
                  />
        ...
        <div className="list-chat">
          {conversationList.map((conversation1) => (
            <div
              className={`chat-box ${conversation1.isSeen ? "" : "not-seen"} ${
                conversation.id === conversation1.id
            ...
            >
              <img
                ...
                src={
                  conversation1.imgUrl ? conversation1.imgUrl : "../user.png"
                }
                alt=""
              />
              <div>
                ...
                <div style={{ fontSize: "12px" }}>
                  {conversation1.lastMessage
                    ? formatDateTime(conversation1.lastMessageTime)
                    : ""}
                </div>
              </div>
            </div>
      ...
      <div className="chat-right">
        {conversation?.id === 0 && (
          <div className="default-chat">Choose the conversation and chat</div>
        )}
        {conversation?.id !== 0 && (
          <div style={{ margin: "20px", height: "100%" }}>
            ...
            <div className="chat-navbar">
              <img
                className="admin-img"
                src={conversation.imgUrl ? conversation.imgUrl : "../user.png"}
                alt=""
              />
            ...
            <div className="send-message-container">
              ...
              />
              <SendOutlined
                style={{ cursor: "pointer", color: "#0ED9F5" }}
                className={`${!tempMessage ? "unactive" : ""}`}
